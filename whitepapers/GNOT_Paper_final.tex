...
\fancyhead[R]{\small\textit{Quoc Viet Tran, 2026}}
\fancyfoot[C]{\thepage}
...
The insight driving the bootstrap model is that exactly three primitive
capabilities are both necessary and sufficient to provision any
computational environment. \texttt{read\_file} and \texttt{write\_file}
together provide full filesystem access---required to inspect state,
write configuration, and install software artifacts. Without either,
the LLM cannot observe or modify the machine's persistent state.
\texttt{execute\_command} provides process execution---required to run
installers, start services, invoke package managers, and call external
APIs. Without it, no software can be activated regardless of what is
written to disk. Removing any one of the three eliminates an
irreducible category of operation; together, they form a minimal
complete basis: any Linux environment can be bootstrapped from them,
and nothing further is needed at seed time. An LLM reasoning over
these three primitives becomes both the provisioning layer and the
orchestration layer simultaneously, without requiring any
infrastructure pre-existing beyond a running GNOT node.
...
route advertisement. Section~\ref{sec:agent} covers the LLM control
plane and ReAct loop. Section~\ref{sec:actions} describes the action
model and plugin contract. Section~\ref{sec:security} discusses the
security model. Section~\ref{sec:bootstrap} explains self-bootstrapping.
Section~\ref{sec:files} describes the file transfer system.
...
a single process on a single host. The author evaluated LangChain as a
foundation for the use cases described in Section~\ref{sec:deployment}
but found it insufficient: there is no mechanism for an LLM to target
a specific remote machine, route through NAT, or provision a new node
at runtime.

\textbf{LlamaIndex} is primarily a retrieval-augmented generation (RAG)
and data indexing framework. Its \texttt{llama-deploy} extension
supports distributing agents as microservices with a message queue, but
requires each service to be pre-defined, containerized, and registered
with a central control plane before execution. The framework assumes a
pre-provisioned, reachable infrastructure; there is no concept of
dynamic node registration, NAT traversal, or LLM-driven self-extension.
The author found LlamaIndex insufficient for the same reason as
LangChain: it cannot accept an LLM instruction to ``install software on
machine X and register it as a new execution node.''

\textbf{OpenClaw}~\cite{openclaw2026} (formerly Clawdbot and Moltbot)
is a personal AI assistant framework that connects an LLM to messaging
platforms (Telegram, WhatsApp, Slack, Discord, and others) and exposes
shell, browser, file, and calendar operations as tools. It gained
significant adoption in early 2026 and is architecturally relevant to
GNOT: both systems route natural-language requests through a local
gateway and dispatch them to execution primitives. OpenClaw's
architectural focus, however, is the \emph{single-machine personal
assistant}: the Gateway is a WebSocket server on one host, skills
execute locally, and there is no concept of remote node registration,
cross-machine routing, or NAT-transparent distributed execution. An
operator cannot instruct an OpenClaw agent to execute a command on a
second private machine and return the result---the class of use cases
GNOT is designed for.

\textbf{ZeroClaw}~\cite{zeroclaw2026} is a Rust-native reimplementation
of the OpenClaw architecture, optimized for resource-constrained
deployments (3.4\,MB binary, sub-10\,ms cold start). It shares
OpenClaw's single-machine execution model and messaging-first interface.
ZeroClaw's design priorities---low memory footprint, security-first
defaults, sandboxed filesystem access---address operational concerns
in single-agent deployments. Like OpenClaw, it does not address
multi-machine mesh execution, dynamic node provisioning, or
NAT-transparent routing.

The common limitation across all four frameworks is the \emph{static
infrastructure assumption}: the LLM is given tools that call
pre-existing, reachable services. GNOT inverts this by making the
infrastructure itself the dynamic artifact: the LLM provisions,
connects, and orchestrates nodes at runtime, using three primitives
that suffice to bootstrap any Linux environment.
...
loop. The pattern allows an LLM to decompose multi-step problems,
course-correct on unexpected results, and produce a final answer only
when it has sufficient information.

GNOT adopts ReAct as the execution model of its \texttt{IntentHandler}.
The key extension over the original formulation is the action target:
where Yao et al.\ dispatch to a local function registry, GNOT
dispatches to a distributed execution mesh that may span multiple
...
\begin{tikzpicture}[
  ...
  box/.style={draw, rounded corners=3pt, font=\small,
              ...
              fill=blue!8, draw=accentblue, text centered},
  gwbox/.style={draw, rounded corners=3pt, font=\small,
              minimum width=5.2cm, minimum height=0.65cm,
              fill=blue!15, draw=accentblue, text centered},
  subbox/.style={draw, rounded corners=3pt, font=\small,
              minimum width=3.0cm, minimum height=0.6cm,
              fill=blue!4, draw=accentblue!60, text centered},
  cloud/.style={draw, ellipse, font=\small,
                fill=gray!10, draw=gray,
                minimum width=2.4cm, minimum height=0.65cm},
  arr/.style={-{Stealth[length=4pt]}, gray!70, line width=0.7pt},
  darr/.style={-{Stealth[length=4pt]}, dashed, gray!55, line width=0.65pt},
  lbl/.style={font=\scriptsize\itshape, text=gray},
]

%% ── Internet + callers ────────────────────────────────────────
\node[cloud] (internet) {Internet};
\node[lbl, above=0.25cm of internet, xshift=-1.4cm] (claude) {Claude Web};
\node[lbl, above=0.25cm of internet, xshift=0cm]    (tg)     {Telegram};
\node[lbl, above=0.25cm of internet, xshift=1.4cm]  (curl)   {HTTP Client};
\draw[darr] (claude.south) -- ++(0,-0.15) -| (internet.north);
\draw[darr] (tg.south)     -- (internet.north);
\draw[darr] (curl.south)   -- ++(0,-0.15) -| (internet.north);

%% ── Gateway ───────────────────────────────────────────────────
\node[gwbox, below=0.8cm of internet] (gw) {\textbf{Gateway Node} (deb-0)};
\draw[arr] (internet.south) -- node[right, lbl]{HTTPS} (gw.north);
\node[lbl, below=0.08cm of gw, xshift=-1.6cm] {GatewayRouter};
\node[lbl, below=0.08cm of gw, xshift=0.1cm]  {IntentHandler};
\node[lbl, below=0.08cm of gw, xshift=1.6cm]  {NodeRegistry};

%% ── Worker nodes ──────────────────────────────────────────────
\node[box, below=2.0cm of gw, xshift=-1.7cm] (w1) {Worker (cen-0)};
\node[box, right=0.4cm of w1]                (w2) {Worker (alm-0)};
\node[box, below=0.7cm of w2]                (w3) {Worker (cfsnk-0)};
\node[subbox, below=0.6cm of w3]             (sub) {sub-node (cfsnk-0a)};

%% ── NAT boundary box drawn around workers ─────────────────────
\begin{scope}[on background layer]
  \node[draw=gray!50, dashed, rounded corners=4pt, line width=0.8pt,
        inner sep=10pt, fill=gray!4,
        fit=(w1)(w2)(w3)(sub)] (natbox) {};
\end{scope}
\node[lbl, above left=0.05cm and 0cm of natbox.north west] {NAT boundary};

%% ── Poll arrows ───────────────────────────────────────────────
\draw[darr] (w1.north) -- node[right, lbl]{poll} (gw.south -| w1.north);
\draw[darr] (w2.north) -- node[right, lbl]{poll} (gw.south -| w2.north);
\draw[darr] (w3.north) -- node[right, lbl]{poll} (gw.south -| w3.north);
\draw[arr]  (sub.north) -- (w3.south);

\end{tikzpicture}
}% end resizebox
\caption{GNOT deployment topology. Workers poll the gateway outbound
(dashed arrows); the gateway never initiates inbound connections to
workers behind NAT\@. Sub-node \texttt{cfsnk-0a} is reachable only
via its parent \texttt{cfsnk-0}.}
\label{fig:topology}
\end{figure}
...
\bibitem{openclaw2026}
  Steinberger, P. et al. (2026).
  \textit{OpenClaw: Open-source autonomous AI agent framework}.
  openclaw/openclaw, GitHub. MIT License.
  \url{https://github.com/openclaw/openclaw}

\bibitem{zeroclaw2026}
  ZeroClaw Labs (2026).
  \textit{ZeroClaw: Fast, small, and fully autonomous AI assistant infrastructure}.
  zeroclaw-labs/zeroclaw, GitHub.
...
\bibitem{gnot2026}
  ...
  gnot-io/gnot, GitHub. Apache License 2.0.
