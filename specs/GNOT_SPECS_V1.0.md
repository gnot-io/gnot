...
**Website:** gnot.io  
**Version:** 1.0 (unified from runtime v5.0 → v5.13b)  
...
**LLM-native distributed execution** in which a large language model acts as both
reasoning engine and orchestrator over a self-extending mesh of heterogeneous compute
nodes. The system bootstraps from a single *seed node* carrying three primitive
capabilities — file read, file write, and command execution — and propagates itself
across any reachable machine through HTTP. Complex, multi-step workflows are expressed
as natural-language intents, decomposed autonomously by an embedded ReAct agent loop,
and executed across nodes that may span multiple network topologies including NAT
boundaries. This document provides the definitive architectural specification covering
node anatomy, communication protocols, routing semantics, capability discovery,
...
3. [Node Anatomy](#3-node-anatomy)  
4. [Action Model](#4-action-model)  
5. [Request and Response Envelope](#5-request-and-response-envelope)  
6. [Job Lifecycle](#6-job-lifecycle)  
7. [Routing Engine](#7-routing-engine)  
8. [Push/Pull Delivery Model](#8-pushpull-delivery-model)  
9. [BGP-style Route Advertisement](#9-bgp-style-route-advertisement)  
10. [Capability Discovery](#10-capability-discovery)  
11. [Authentication and Authorization](#11-authentication-and-authorization)  
12. [Intent Agent Loop](#12-intent-agent-loop)  
13. [Session Management](#13-session-management)  
14. [File Transfer System](#14-file-transfer-system)  
15. [Credential System](#15-credential-system)  
16. [Self-Bootstrapping Mechanism](#16-self-bootstrapping-mechanism)  
17. [Observability and Health](#17-observability-and-health)  
18. [Node Configuration Reference](#18-node-configuration-reference)  
...
23. [Evolution History and Design Decisions](#23-evolution-history-and-design-decisions)  
...
> **Maximal minimalism at origin — unbounded extensibility at runtime.**

GNOT begins as a single node with three irreducible capabilities. Everything else —
additional nodes, specialized compute, new actions, complex orchestration — emerges
at runtime through the reasoning of a large language model acting upon those three
primitives.

This philosophy has three implications:

1. **No provisioning layer.** The LLM *is* the provisioning layer. It writes
   configuration files, installs software, starts processes, and registers new nodes
   without any external orchestration tooling.

2. **No schema at origin.** The seed node does not know what it will eventually do.
   Capability schemas emerge as new actions are created.

3. **No topology assumption.** The routing layer adapts to the actual network topology
   at registration time, not at design time.

### 1.2 LLM as First-Class Runtime Component

In conventional distributed systems, an LLM might sit outside the infrastructure as
a chat interface. In GNOT, the LLM is *inside* the execution path:

```
User intent
    ↓
LLM reasoning (ReAct loop)
    ↓
Mesh action dispatch
    ↓
...
The LLM is not a wrapper — it generates every orchestration decision, including which
node to target, which action to invoke, what parameters to construct, and how to
interpret results. This is what distinguishes GNOT from LLM-assisted automation:
the LLM is the control plane.

### 1.3 Transparency Across Delivery Modes
...
|------|-------------|-----------|
| L1 — Credential re-encryption | Changing `credential_encryption_key` silently drops existing ciphertexts rather than re-encrypting | Re-encrypt before rotation by exporting and re-importing credentials |
| L2 — Job queue persistence | Pull queue is in-memory; gateway restart loses undelivered jobs | `pull_job_timeout_seconds` bounds exposure window |
| L3 — Token-to-node audit logging | Auth logs do not map token → node identity | Operator can correlate from tokens.env |
| L4 — Container isolation | `execute_command` has full host filesystem access | Run each node as non-root with restricted permissions |
| L5 — Graceful shutdown route withdrawal | Stopping a node does not send explicit route withdrawal; peers detect via heartbeat timeout | Acceptable for most deployments; heartbeat_timeout_seconds bounds detection delay |
| L6 — Streaming intent response | `POST /intent` returns a single JSON response after all turns complete | Use polling or SSE (planned) for long workflows |
| L7 — Hot-reload configuration | Token additions/removals require node restart | Restart is fast; acceptable for current scale |

### 24.2 Future Work

| Priority | Feature | Description |
|----------|---------|-------------|
| P1 | Streaming intent (SSE) | `POST /intent` returns `text/event-stream` with turn-by-turn updates |
| P1 | Credential re-encryption on key rotation | `credential_store.rekey(new_key)` re-encrypts all stored values |
| P2 | Token-to-node audit logging | Map token fingerprint → node_id in access logs |
| P2 | Hot-reload config | SIGHUP or watch `node.yaml` for token list changes without restart |
| P2 | Job queue persistence | SQLite-backed pull queue surviving gateway restarts |
| P3 | Container isolation | Optionally run each node's actions in a container via Docker or nsjail |
| P3 | Multi-turn file context | Large file analysis across multiple turns via chunked read_file |
| P3 | Explicit WITHDRAW on shutdown | Node sends withdrawal signal to gateway before process exit |
| P3 | Metrics endpoint | Prometheus `/metrics` exposing job throughput, queue depths, latency percentiles |
| P3 | Web UI | Read-only dashboard showing mesh topology and job history |
